        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Building className="size-3" />{job.type}</span>
              <span className="font-medium text-foreground">{job.salary}</span>
            </div>
          </div>
        </div>
        <div className="mt-4 flex flex-wrap gap-1.5">{job.tags.map((t) => <Badge key={t} variant="secondary">{t}</Badge>)}</div>
        <div className="mt-6 flex gap-2">
          <Button className="flex-1 gradient-brand text-white border-0" onClick={() => toast.success("Application submitted!")}>Apply now</Button>
          <Button variant="outline"><Bookmark className="size-4 mr-1.5" />Save</Button>
        </div>
      </Card>
      <Card className="p-6 space-y-4">
        <div><h3 className="font-semibold mb-2">About the role</h3><p className="text-sm text-muted-foreground">{job.description} You'll partner with product, design, and engineering leaders to ship features used by millions.</p></div>
        <div><h3 className="font-semibold mb-2">What you'll do</h3>
        ...
        <div><h3 className="font-semibold mb-2">Requirements</h3>
          <ul className="text-sm text-muted-foreground space-y-1 list-disc pl-5">
