...
export default function Register() {
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm<Form>({ resolver: zodResolver(schema), defaultValues: { role: "student" } });
    ...
    <Card className="p-8 border-0 shadow-none">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
        ...
        <div>
          ...
          <Input {...register("email")} type="email" className="mt-1" placeholder="you@campus.edu" />
          {errors.email && <p className="text-xs text-destructive mt-1">{errors.email.message}</p>}
        </div>
        <div>
          <label className="text-sm font-medium">Password</label>
          <Input {...register("password")} type="password" className="mt-1" placeholder="At least 6 characters" />
