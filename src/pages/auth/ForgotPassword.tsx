  ...
  return (
    <Card className="p-8 border-0 shadow-none">
      ...
      <form onSubmit={(e) => { e.preventDefault(); toast.success("Reset link sent!"); }} className="mt-6 space-y-4">
        ...
        <Button type="submit" className="w-full gradient-brand text-white border-0">Send reset link</Button>
      </form>
      <p className="text-center text-sm text-muted-foreground mt-6"><Link to="/login" className="text-primary font-medium hover:underline">← Back to sign in</Link></p>
    </Card>
  );
}
