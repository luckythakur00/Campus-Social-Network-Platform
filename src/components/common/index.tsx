...
export function StatCard({ label, value, icon: Icon, hint, tone = "primary" }: { label: string; value: string | number; icon: LucideIcon; hint?: string; tone?: "primary" | "success" | "warning" | "secondary" }) {
...
export function EmptyState({ title, description, icon: Icon }: { title: string; description?: string; icon?: LucideIcon }) {
  return (
    <div className="text-center py-16 border border-dashed rounded-2xl">
      {Icon && <div className="mx-auto size-12 rounded-2xl bg-muted grid place-items-center mb-3"><Icon className="size-6 text-muted-foreground" /></div>}
